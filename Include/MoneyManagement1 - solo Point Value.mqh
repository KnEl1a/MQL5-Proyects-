...
double MoneyManagement(string pSymbol,double pFixedVol,double pPercent,int pStopPoints)
{
	...
	if(pPercent > 0 && pStopPoints > 0)
	{
		...
		//double PointValue = SymbolInfoDouble(pSymbol,SYMBOL_TRADE_TICK_VALUE);
		double PointValue = SymbolInfoDouble(pSymbol,SYMBOL_POINT);
		
		tradeSize = (margin / pStopPoints) / PointValue;
...
{
	double stopDiff = MathAbs(pStopPrice - pOrderPrice); // esto lo establecemos en nuestro programa principal para que no tengamos problemas 
	double getPoint = SymbolInfoDouble(pSymbol,SYMBOL_POINT);
	double priceToPoint = stopDiff / getPoint;
	//double priceToPoint = stopDiff; // borramos getpoint porque creo que asi nos quedamos con la diferencia en putnos original y asi no la pasamos a ticks
	return(priceToPoint);
}