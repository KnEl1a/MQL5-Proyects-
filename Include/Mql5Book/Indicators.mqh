...
class CIndicator
{
	...
	public:
		CIndicator(void);//constructor
		double Main(int pShift=0);
		void Release();
		virtual int Init() { return(handle); } // con virtual podemos realizar polimorfismo, o sea cada subclase puede acceder y realizar a su manera una tarea determinada.
};

CIndicator::CIndicator(void)
{
	ArraySetAsSeries(main,true);//constructor
}
...
{
	IndicatorRelease(handle); // bool, Elimina el manejador del indicador y libera la parte calculadora del indicador si nadie la usa.
}
