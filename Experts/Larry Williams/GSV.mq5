...
Este venia mal con el calculo venia fallado de fabrica asi que lo corregi la clave estaba en el if 
if(rates[0].open + Rango_GSV_compra() <= rates[0].close && operacion_cerrada()) esta es la manera correcta de hacer el gsv
...
void OnTick ()
{
   ...
   if((rates[0].open - Rango_GSV_venta()) == bid && operacion_cerrada()) // aca me quede en sl , que sera la orden contraria para este caso basico , despues tomare el concepto y lo estudiare a fondo.
   {
...
{
   double rango_compra =  ((MathAbs(rates[4].high - rates[4].open) + MathAbs(rates[3].high - rates[3].open) + MathAbs(rates[2].high - rates[2].open) + MathAbs(rates[1].high - rates[1].open))/4)*(swing_percent_value/100);
