...
//entrar cuando el rsi de 2 periodos este en niveles de 5 o 95
...
void OnInit()
  {
   rsi_h=iRSI(_Symbol, _Period, 2,PRICE_CLOSE);
...
//if((rates[0].open + Rango_GSV_compra()) == ask && operacion_cerrada())
   if(rates[0].open + Rango_GSV_compra() <= rates[0].close && rsi[1] <= 5 && operacion_cerrada())
     {
      trade.Buy(0.1,_Symbol,ask, (rates[0].open - Rango_GSV_venta()), (rates[0].open + Rango_GSV_compra()* 2.5));
      trade_ticket = trade.ResultOrder();
     }

   if((rates[0].open - Rango_GSV_venta()) >= rates[0].close && rsi[1] >= 95 && operacion_cerrada()) // aca me quede en sl , que sera la orden contraria para este caso basico , despues tomare el concepto y lo estudiare a fondo.
     {
